...
stock::stock(QString id,QString nom,int quantite,QString couleur,int poids,int taille,QString genre,QDate datep)
{
    this->id=id;
    this->nom=nom;
...
QString stock::getcouleur(){return couleur;}
...
void stock::setcouleur(QString couleur){this->couleur=couleur;}
...
query.prepare("insert into STOCK (id,nom,quantite,couleur,poids,taille,genre,datep)" "values (:id,:nom,:quantite,:couleur,:poids,:taille,:genre,:datep)");
//bindValue+requete
...
bool stock::modifier(){
    ...
    QString poids_string = QString::number(poids);
    QString taille_string = QString::number(taille);
    query.prepare("UPDATE stock set id=:id,nom=:nom,quantite=:quantite,couleur=:couleur,poids=:poids,taille=:taille,genre=:genre,datep=:datep WHERE id=:id");
